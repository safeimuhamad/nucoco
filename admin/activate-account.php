...
$user = $token_hash !== ''
    ? db_select_one("SELECT id, name, email FROM users WHERE activation_token_hash = ? AND activation_expires_at >= NOW() LIMIT 1", 's', [$token_hash])
    : null;
$error = '';
$success = false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$user) {
        ...
        db_update(
            "UPDATE users
             SET password = ?, status = 'active', activated_at = NOW(), activation_token_hash = NULL, activation_expires_at = NULL,
...
<head>
    ...
    <title>Activate Account - Nucoco</title>
            ...
            <div class="m-lg-auto my-auto w-930 py-4">
                <div class="card bg-white border rounded-10 border-white py-5 px-5">
                    <div class="p-md-5 p-4">
                        <h3 class="fs-26 fw-medium mb-2">Activate Account</h3>
                        <?php if ($success): ?>
                            <div class="alert alert-success">Your account is active. You can sign in now.</div>
                            <a href="<?= admin_url('login') ?>" class="btn btn-primary text-white">Sign In</a>
                        <?php elseif (!$user): ?>
                            <div class="alert alert-danger">Activation link is invalid or expired.</div>
                            <a href="<?= admin_url('login') ?>" class="btn btn-primary text-white">Back to Sign In</a>
                        <?php else: ?>
                            <p class="text-secondary mb-4">Hello <?= htmlspecialchars($user['name']) ?>, please create your password.</p>
                            ...
                            <form method="POST">
                                ...
                                <button class="btn btn-primary text-white w-100" type="submit">Activate Account</button>
                            </form>
                        <?php endif; ?>
                    </div>
                </div>

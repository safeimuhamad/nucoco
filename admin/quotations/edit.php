                                ...
                                <select name="items[<?= $i ?>][catalog_key]" class="form-select catalog-select">
                                    ...
                                    <optgroup label="Products">
                                        <?php foreach ($products as $product): ?>
                                            <option value="product:<?= (int) $product['id'] ?>" data-name="<?= htmlspecialchars($product['name']) ?>" data-price="<?= (float) ($product['price'] ?? 0) ?>" data-description="<?= htmlspecialchars(quotation_catalog_description($product['name'], $product['quotation_description'] ?? '', $language, 'product')) ?>">Product - <?= htmlspecialchars($product['name']) ?></option>
                                        <?php endforeach; ?>
                                    </optgroup>
                                    <optgroup label="Services">
                                        <?php foreach ($services as $service): ?>
                                            <option value="service:<?= (int) $service['id'] ?>" data-name="<?= htmlspecialchars($service['title']) ?>" data-price="0" data-description="<?= htmlspecialchars(quotation_catalog_description($service['title'], $service['quotation_description'] ?? '', $language, 'service')) ?>">Service - <?= htmlspecialchars($service['title']) ?></option>
                                        <?php endforeach; ?>
                                    </optgroup>
                                </select>
                            </td>
